...
): { data?: string; transcript?: string } | undefined {
  if (!delta) {
    return undefined;
  }

  const audioValue = Reflect.get(delta, "audio");
  if (!audioValue || typeof audioValue !== "object") {
    return undefined;
  }

  const dataValue = Reflect.get(audioValue, "data");
  const transcriptValue = Reflect.get(audioValue, "transcript");
  const data = typeof dataValue === "string" ? dataValue : undefined;
  const transcript =
    typeof transcriptValue === "string" ? transcriptValue : undefined;
...
export async function voiceChat(
  ...
  const transcript =
    message?.audio?.transcript ?? getMessageContentText(message?.content);
...
export async function voiceChatStream(
  ...
  return (async function* () {
    for await (const chunk of stream) {
      const audioDelta = getAudioDelta(chunk.choices?.[0]?.delta);
      if (!audioDelta) continue;
      if (audioDelta.transcript) {
        yield { type: "transcript", data: audioDelta.transcript };
      }
      if (audioDelta.data) {
        yield { type: "audio", data: audioDelta.data };
      }
    }
  })();
}
